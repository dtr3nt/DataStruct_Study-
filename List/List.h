...
template <class List_entry>
Error_code List<List_entry>::double_insert(int position, 
    const List_entry &x1, const List_entry &x2)
/*  
Post: If the List is not full and 0 <= position <= n,
where n is the number of entries in the List, the 
function succeeds:
    Any entry formerly at position and all subsequent
entries have their position numbers increased by 2, and 
x1 and x2 are inserted at position and position + 1, 
...
*/
{
   if (position < 0 || position > count)
      return range_err;
   Node<List_entry> *new_nodeA, *new_nodeB, *previous, *following;
   if (position > 0) {
      previous = set_position(position - 1);
      following = previous->next;
   }
   else following = head;
   new_nodeA = new Node<List_entry>(x1, following);
   new_nodeB = new Node<List_entry>(x2, new_nodeA->next);
   if (new_nodeA == NULL)
      return overflow;
   if (position == 0)
   {
      head = new_nodeA;
      new_nodeA->next = new_nodeB;
   }
   else
   {
      previous->next = new_nodeA;
      new_nodeA->next = new_nodeB;
      new_nodeB->next = following;
   }
   count += 2;
   return success; 
}